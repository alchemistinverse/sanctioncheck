  ...
  for (const nb of nameBlocks) {
    ...
    if (firstName && lastName) {
      ...
      display = fullName.includes(',')
        ? fullName.split(',').map(s => s.trim()).reverse().join(' ')
        : fullName
    }

    if (isPrimary && !primaryName) {
      primaryName = display || fullName
    } else {
      if (display && display !== primaryName) aliases.push(display)
      if (fullName && fullName !== display && fullName !== primaryName) aliases.push(fullName)
    }
  }
